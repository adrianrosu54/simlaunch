...
import {
    $unitView, fromDisplay, setUnitView, toDisplay,
    type UnitCategory
} from "../../stores/units";
            ...
            <div className="flex flex-row overflow-hidden bg-clk-background border-4 border-clk-primary rounded-3xl">
                ...
                {units && (
                    units.map((value, index, array) =>
                        <button key={value} className={`px-3 w-20 opacity-90 hover:opacity-100 hover:cursor-pointer\
                            ${value === currentUnit ? "bg-clk-accent" : "bg-clk-secondary"}\
                            ${index === 0 ? "rounded-l-xl pl-3" : ""}\
                            ${index === array.length - 1 ? "rounded-r-xl pr-3" : ""}`}
                            onClick={(e) => {
                                e.preventDefault();
